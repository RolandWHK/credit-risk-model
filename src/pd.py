...
Original file is located at
    https://colab.research.google.com/drive/1yYnbao1ajpgTIhjGRVWB2xqwaBxvMVwf
"""

import numpy as np
import pandas as pd

loan_data_backup = pd.read_csv('/content/sample_data/3.1 loan_data_2007_2014.csv')

loan_data = loan_data_backup.copy()
loan_data.drop(axis=1, columns="Unnamed: 0", inplace=True)

pd.options.display.max_columns = None

loan_data

loan_data['emp_length_int'] = loan_data['emp_length'].replace({
    # Replace the actual categories with numbers first
    r'< 1 year': '0',
    r'n/a': '0',
    # Replace '10+ years' with just '10', and remove other ' years' / ' year' text
    r'(\D|\+)*years?': '', # Removes ' years', ' year', '+ years', etc.
    r'\+': '' # Ensure any remaining '+' is removed
}, regex=True)
...
loan_data['term_int'] = loan_data['term'].replace({
    r'n/a': '0',
    r'(\D|\+)*months?': ''
}, regex=True)
...
loan_data['mths_since_earliest_cr_line'] = round(
    (pd.to_datetime('2017-12-01') - loan_data['earliest_cr_line_date']).dt.days / 30.4375
)

print(loan_data[['earliest_cr_line_date', 'mths_since_earliest_cr_line']].head())
...
loan_data['mths_since_issue_d'] = round(
    (pd.to_datetime('2017-12-01') - loan_data['issue_d_date']).dt.days / 30.4375
)
...
cleaned_data = loan_data.to_csv("loan_data_2007_2014_preprocessed.csv", index = False)
...
def woe_discrete(df, discrete_variable_name, good_bad_variable_df):
  ...
  df = df.iloc[:, [0, 1, 3]]
  df.columns.values[1] = 'n_obs'
  df.columns.values[2] = 'prop_good'
...
print(int(df_inputs_prepr['mths_since_issue_d'].max()))
...
print(int(df_inputs_prepr['mths_since_earliest_cr_line'].max()))
